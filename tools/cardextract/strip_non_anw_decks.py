...
"A New World" deck coexist, the in-game Deck Builder defaults to
...
Warns if a file has no "A New World" deck (leaves it untouched).
"""
...
def process(path: Path) -> str:
    ...
    for indent, body in decks:
        if re.search(r"<name>\s*A New World\s*</name>", body):
            ll = (indent, body)
            break
    if ll is None:
        return "no_ll_found"
