// ANW Age Capture Test -- human P1 age-up screenshot arena
// Derived from anwHubTest. Player 1 = human-playable civ at center island
// with TC + starting units + 100k resources at T+3s for instant age-up.
// Slots 2-8 = AI in 7 wedge compartments (physically isolated by cliffs/sea).
//
// Purpose: let a human player drive all age-up politician dialogs (ages II-V)
// so the "Select a Politician" screen can be screenshotted for each civ.
//
// Status (derived 2026-06-01 from anwHubTest v5+):
//   - All terrain/AI compartments unchanged from anwHubTest.
//   - P1 gets TC + startingUnits at center island (0.5, 0.5).
//   - ageCaptureP1Boost trigger grants 100k Food/Wood/Gold at T+3s.
//   - Auto-end at gHubTestEndSeconds=1200 gives ample time for age-ups.
//
...
void main(void)
{
    ...
    rmEchoInfo("[AGECAPTURE] script started");
    ...
    // ---- P1 playable base (Age Capture Test) --------------------------------
    // Player 1 is human-controlled. Place TC + starting units at the center
    ...
    // -------------------------------------------------------------------------
    // -------------------------------------------------------------------------
    // ageCaptureP1Boost: grant 100k Food/Wood/Gold to P1 at T+3s so the
    // human player can immediately afford any age-up politician path.
    // -------------------------------------------------------------------------
    rmCreateTrigger("ageCaptureP1Boost");
    rmSwitchToTrigger(rmTriggerID("ageCaptureP1Boost"));
    rmSetTriggerPriority(4);
    rmSetTriggerActive(true);
    rmSetTriggerRunImmediately(true);
    ...
    rmSetTriggerEffectParam("ResName", "Food", false);
    rmSetTriggerEffectParamInt("Amount", 100000, false);
    rmAddTriggerEffect("Grant Resources");
    rmSetTriggerEffectParamInt("PlayerID", 1, false);
    rmSetTriggerEffectParam("ResName", "Wood", false);
    rmSetTriggerEffectParamInt("Amount", 100000, false);
