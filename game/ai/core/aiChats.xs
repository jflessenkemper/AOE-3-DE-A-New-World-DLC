...
minInterval 10
{
   ...
   if (haveLOS == true)
   {
      float d = distance(tcID1Location, tcID2Location);
      if (d <= 100.0)
      { // Close together.
...
minInterval 60
{
   ...
   { 
      ...
      if ((shouldResignCount < 3) &&
          ((xsGetTime() - shouldResignLastTime) > 20 * 60 * 1000)) // Haven't done it 3 times or within 3 minutes. Changed to 20...
      {
...
minInterval 5
{
   ...
   {  // We see one of this player's units for the first time... let's do some analysis on it.
      ...
      if (distance(kbBaseGetLocation(cMyID, kbBaseGetMainID(cMyID)), unitPosition) < 60.0)
      {
...
void commHandler(int chatID = -1, int statementID = -1)
{
   ...
   switch (verb) // Parse this message starting with the verb.
   {
      case cPlayerChatVerbAttack:
      {
         ...
         int numUnits = kbUnitCount(cMyID, cUnitTypeLogicalTypeLandMilitary, cUnitStateAlive); // Enhanced 
         ...
         if (currentFreeMilitaryPop < 10) // Enhanced
         {
         ...
         if (isDefending() == true) // Enhanced - Just check if under attack instead.
         {
