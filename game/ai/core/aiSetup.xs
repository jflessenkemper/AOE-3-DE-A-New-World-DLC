...
void initArrays(void)
{
      ...
      case cDifficultyHard: // Hard.
   {
      ...
      else
      {
         ...
         xsArraySetInt(gTargetSettlerCounts, cAge5, 80); // Changed these from 99..
      }
      break;
   }
   ...
   if (civIsEuropean() == true && civIsRevolution() == false)
   {
   ...
   gMapNames = xsArrayCreateString(237, "", "Map names");
   ...
   xsArraySetString(gMapNames, 234, "euMediterranean");
   xsArraySetString(gMapNames, 235, "euMediterraneanlarge");
   xsArraySetString(gMapNames, 236, "euBaltic");
   xsArraySetString(gMapNames, 237, "euBalticlarge");
   
   // List above is up to date for the Italy/Malta release. Added missing entries for the Enhanced mod... 8/20/2023
}
...
void analyzeGameSettingsAndType()
{
      ...
      case cDifficultyHard: // Hard.
   {
      ...
      else
      {
         gMaxPop = maxPop;
         aiSetMicroFlags(cMicroLevelHigh);
         gAttackMissionInterval = 90000; // 2.5 Minutes. // Enhanced - Changed to 1.5 to match the others...
      }
      break;
   }
   ...
   {
      gMaxPop = maxPop;
      gAttackMissionInterval = 120000; // 2 Minutes. // Enhanced - Changed to 1.5...
      kbSetPlayerHandicap(cMyID, startingHandicap * 1.35); // +35% Boost.
      aiSetMicroFlags(cMicroLevelHigh);
      break;
   }
      case cDifficultyExtreme: // Extreme.
   {
      gMaxPop = maxPop;
      gAttackMissionInterval = 120000; // 2 Minutes. // Enhanced - Changed to 1.5...
      kbSetPlayerHandicap(cMyID, startingHandicap * 1.50); // +50% Boost.
      aiSetMicroFlags(cMicroLevelHigh);
      break;
   }
   }
...
void initPersonality(void)
{
   ...
   case cCivRussians: // Ivan:  Infantry oriented rusher
   {
      ...
      btOffenseDefense = 0.5;
      btBiasCav = 0.6; // Enhanced - Changed from -0.4
      btBiasInf = 0.4;
      btBiasArt = -0.4; // Enhanced - Changed from 0.0
      btBiasNative = 0.0;
         btBiasTrade = 0.5;
      break;
   }
