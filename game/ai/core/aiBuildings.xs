...
//==============================================================================
/* selectTeepeeBuildPlanPosition
   Placement algorithm is brain-dead simple.  Check a point that is mid-edge or a
   corner of a square around the base center.  Look for a nearby teepee.  If none,
   do a tight build plan.  If there is one, try again.    If no luck, try a build
   plan that just avoids other teepees.
*/
//==============================================================================
void selectTeepeeBuildPlanPosition(int buildPlan = -1, int baseID = -1)
{
   int teepeeBL = kbGetBuildLimit(cMyID, cUnitTypeTeepee);
   int numAttempts = 3 * teepeeBL / 2;
   vector testVec = cInvalidVector;
   static vector baseVec = cInvalidVector;
   static vector startingVec = cInvalidVector;
   int numTestVecs = 5 * teepeeBL / 4;
   float teepeeAngle = (2.0 * PI) / numTestVecs;
   // Mid- and corner-spots on a square with 'radius' spacingDistance, i.e. each side is 2 * spacingDistance.
   float spacingDistance = 12 * sin((PI - teepeeAngle) / 2.0) / sin(teepeeAngle); 
   float exclusionRadius = spacingDistance / 2.0;

   static int teepeeSearch = -1;
   bool success = false;
   ...
   {
      testVec = rotateByReferencePoint(baseVec, startingVec - baseVec, teepeeAngle * aiRandInt(numTestVecs));
      debugBuildings("Testing teepee location at: " + testVec);
      if (teepeeSearch < 0)
      { // init
         teepeeSearch = kbUnitQueryCreate("teepee placement search");
         kbUnitQuerySetPlayerRelation(teepeeSearch, cPlayerRelationAny);
         kbUnitQuerySetUnitType(teepeeSearch, cUnitTypeTeepee);
         kbUnitQuerySetState(teepeeSearch, cUnitStateABQ);
      }
      kbUnitQuerySetPosition(teepeeSearch, testVec);
      kbUnitQuerySetMaximumDistance(teepeeSearch, exclusionRadius);
      kbUnitQueryResetResults(teepeeSearch);
      if (kbUnitQueryExecute(teepeeSearch) < 1)
   ...
   // Add position influence for nearby teepees, this doesn't work when the allied teepee is a different PUID.
   aiPlanSetVariableInt(buildPlan, cBuildPlanInfluenceUnitTypeID, 0, cUnitTypeTeepee);
   aiPlanSetVariableFloat(buildPlan, cBuildPlanInfluenceUnitDistance, 0, spacingDistance);
   aiPlanSetVariableFloat(buildPlan, cBuildPlanInfluenceUnitValue, 0, -20.0);             // -20 points per teepee
   aiPlanSetVariableInt(buildPlan, cBuildPlanInfluenceUnitFalloff, 0, cBPIFalloffLinear); // Linear slope falloff
   ...
   debugBuildings("Building a Teepee at location: " + testVec);
}
...
void selectTowerBuildPlanPosition(int buildPlan = -1, int baseID = -1)
{
   ...
   bool dockTowerInProgress = false;
   int docksQuery = createSimpleUnitQuery(gDockUnit, cMyID, cUnitStateABQ);
   int docksFound = kbUnitQueryExecute(docksQuery);

   if ((gIslandMap == true || gNavyMap == true) && docksFound > 0)
   {
      for (int i = 0; i < docksFound; i++)
      {
         vector dockPos = kbUnitGetPosition(kbUnitQueryGetResult(docksQuery, i));
         int towersQuery = createSimpleUnitQuery(gTowerUnit, cMyID, cUnitStateABQ);
         kbUnitQuerySetPosition(towersQuery, dockPos);
         kbUnitQuerySetMaximumDistance(towersQuery, 12.0);
         int towersFound = kbUnitQueryExecute(towersQuery);

         if (towersFound <= 0) {
               vector direction = coastalEdgeVector(baseVec, dockPos) - baseVec;
               float length = xsVectorLength(direction) - 7.0;
               if (length < 0) length = 0; // Ensure the length is not negative.
               direction = xsVectorNormalize(direction);
               testVec = baseVec + direction * length;
               exclusionRadius = 7.0;
               success = true;
               dockTowerInProgress = true;
               break;
         }
      }
   }

   // Make sure we protect our factories with some towers too.
   ...
   if (factoriesFound > 0 && dockTowerInProgress == false) {
      for (i = 0; i < factoriesFound; i++)
      {
         vector factoryPos = kbUnitGetPosition(kbUnitQueryGetResult(factoriesQuery, i));
         towersQuery = createSimpleUnitQuery(gTowerUnit, cMyID, cUnitStateABQ);
         kbUnitQuerySetPosition(towersQuery, factoryPos);
         kbUnitQuerySetMaximumDistance(towersQuery, 12.0);
         towersFound = kbUnitQueryExecute(towersQuery);

         if (towersFound <= 0) {
               direction = factoryPos - baseVec;
               length = xsVectorLength(direction) - 5.0;
               if (length < 0) length = 0; // Ensure the length is not negative.
               direction = xsVectorNormalize(direction);
               testVec = baseVec + direction * length;
...
bool selectBuildPlanPosition(int planID = -1, int puid = -1, int baseID = -1)
{
   ...
   switch (puid)
   {
      ...
      {
         selectTeepeeBuildPlanPosition(planID, baseID);
         break;
      }
      ...
      case cUnitTypedePort:
      {
         ...
         aiPlanSetVariableVector(planID, cBuildPlanDockPlacementPoint, 1, gNavyVec); // One point at spawn flag or closest fish.
         break;
      }
      case cUnitTypeBank:
      case cUnitTypeMarket:
...
vector selectForwardBaseLocation(void)
{
   ...
   else // Enemy TC found.
   {
      ...
      radius = 130.0 + kbBaseGetDistance(enemyPlayer, kbUnitGetBaseID(enemyTC));
   }
...
minInterval 5
{
   ...
   if ((((gTimeToFish == true) || (age >= cAge3)) && (dockCount < 1)) ||
      ((cDifficultyCurrent >= cDifficultyHard) && (age >= cAge4) && (dockCount < 2)))
   {
      if ((aiPlanGetIDByTypeAndVariableType(cPlanBuild, cBuildPlanBuildingTypeID, gDockUnit) < 0) &&
          ((gRevolutionType & cRevolutionFinland) == 0))
      { 
         planID = createSimpleBuildPlan(gDockUnit, 1, 70, true, cMilitaryEscrowID, mainBaseID, 1);
         aiPlanSetDesiredResourcePriority(planID, 60);
      }
   }
   ...
   {
      int churchLimit = kbGetBuildLimit(cMyID, cUnitTypeChurch); // Enhanced

      planID = aiPlanGetIDByTypeAndVariableType(cPlanBuild, cBuildPlanBuildingTypeID, cUnitTypeChurch);
      if ((planID < 0) && (kbUnitCount(cMyID, cUnitTypeChurch, cUnitStateAlive) < churchLimit) &&
          (cDifficultyCurrent >= cDifficultyHard))
      {
...
minInterval 40
{
   ...
   // Prevent the AI from spending too much time building towers early on. Enhanced
   if (age < cAge4 && cvMaxAge > cAge3) {
      if (age == cAge2) {
         towersWanted = 1;
      } else {
         towersWanted = 2;
...
void updateWantedTowers()
{
   ...
   else if (cDifficultyCurrent < cDifficultyHard) // Moderate / Hard. Only less than hard now. Enhanced
   {
      if (gNumTowers > 4)
      {
         gNumTowers = 4;
...
minInterval 10
{
   ...
   if (wallPlanID != -1)
   {
      ...
      aiPlanSetDesiredPriority(wallPlanID, 99);
