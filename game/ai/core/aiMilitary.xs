...
minInterval 28
{
   ...
   if (gLandUnitPicker != -1)
   {
      ...
      for (i = 0; < gNumArmyUnitTypes)
      {
         ...
         if (trainBuildingPUID >= 0 && age >= cAge3)
         {
            ...
            if (upgradeTechID >= 0)
            {
               ...
               {
                  aiPlanSetDesiredResourcePriority(upgradePlanID, 65 - (5 - totalValue / 200));
               }
               else
               {
                  aiPlanSetDesiredResourcePriority(upgradePlanID, 70); // Enhanced
               }
            }
         }
      ...
      if (cDifficultyCurrent >= cDifficultyHard) // Enhanced - was cDifficultyExpert
      {
         ...
         {
            // If we have an island base, let's use it.
            if (gActiveIslandInvasion == true && gIslandBaseID > 0) {
               baseID = gIslandBaseID;
            } else {
...
minInterval 15
{
   ...
   // "real" defend plan.
   if ((aiTreatyActive() == true) || (gDefenseReflexBaseID == mainBaseID) || (aiPlanGetActualResourcePriority(gAgeUpResearchPlan) >= 90) ||
       (isDefending() == true)) // Enhanced - Changed to defense only.
   {
   ...
   for (player = currentPlayer; < cNumberPlayers)
   {
      ...
      for (baseIndex = currentBaseIndex; < numberBases)
      {
         ...
         {
            // Avoid attacking until 3 minutes passed after aging up. - Enhanced, this was 5 before.
            if ((btRushBoom <= -0.5 && kbGetAge() < cAge4) || (btRushBoom <= 0.0 && kbGetAge() < cAge3))
            {
               if ((currentTime - gAgeUpTime < 180000))
               {
                  shouldAttack = false;
               }
            }
   ...
   if (targetBaseID < 0 || targetShouldAttack == false)
   {
      ...
      else if (targetPlayer > 0)
      {
         ...
         if (targetIsEnemy == true)
         {
            ...
            // Enhanced - Intervals are a bit too predictable.
            if((currentTime - gLastAttackMissionTime < 2 * gAttackMissionInterval) || checkMilitarySize())
            {
               return;
            }
         }
   ...
   if (targetIsEnemy == true)
   {
      ...
      aiPlanSetVariableVector(planID, cCombatPlanTargetPoint, 0, targetBaseLocation); // Enhanced
      ...
      {
         if (cDifficultyCurrent >= cDifficultyHard) // Enhanced - Once again, was expert.
         {
         ...
         aiPlanSetVariableInt(planID, cCombatPlanRetreatMode, 0, cCombatPlanRetreatModeOutnumbered);
            if (kbBaseGetActive(cMyID, gIslandBaseID) == true) {
               updateMilitaryTrainPlanBuildings(gIslandBaseID);
            } else {
      ...
      if (targetBaseID < 0)
      {
         ...
         {
            sendStatement(cPlayerRelationAllyExcludingSelf, cAICommPromptToAllyIWillAttackEnemyTown, targetBaseLocation);
         }
         else
         {
            sendStatement(cPlayerRelationAllyExcludingSelf, cAICommPromptToAllyIWillAttackEnemyBase, targetBaseLocation);
         }
         if (getUnitCountByLocation(cUnitTypeTradingPost, cPlayerRelationEnemyNotGaia, cUnitStateAlive, targetBaseLocation, 30.0) > 0)
         {
            sendStatement(cPlayerRelationAllyExcludingSelf, cAICommPromptToAllyIWillAttackTradeSite, targetBaseLocation);
         }
         else if (getUnitCountByLocation(gEconUnit, cPlayerRelationEnemyNotGaia, cUnitStateAlive, targetBaseLocation, 25.0) > 7)
         {
...
minInterval 30
{
   ...
   int targetDockID = getClosestUnitByLocation(cUnitTypeAbstractDock, cPlayerRelationEnemyNotGaia, cUnitStateAlive,
   ...
   if (targetDockID >= 0) // There's something to attack.
   {
      ...
      if (cDifficultyCurrent >= cDifficultyHard)
      {
         if (cDifficultyCurrent >= cDifficultyHard) // Enhanced - Was gDifficultyExpert
         {
...
minInterval 10
{
   if (gNavyDefendPlan < 0) // First run, create a persistent defend plan.
   {
      ...
      aiPlanSetVariableFloat(gNavyDefendPlan, cCombatPlanGatherDistance, 0, 100.0); // Enhanced
...
minInterval 30
{
   ...
   if (kbUnitCount(cMyID, gDockUnit, cUnitStateABQ) < 5)   // Changed from 1...
   {  // No dock.  If no fishing plan, and no dock plan, then start one...otherwise just wait.
      ...
      // Priority.
      aiPlanSetDesiredPriority(dockPlan, 100); // Changed from 80...
      // Mil vs. Econ.
      ...
      if (kbUnitCount(cMyID, cUnitTypeYPDockWagon, cUnitStateAlive) > 0) {
        aiPlanAddUnitType(dockPlan, cUnitTypeYPDockWagon, 1, 1, 1);
         ...
         aiPlanAddUnitType(dockPlan, cUnitTypedeDockWagon, 1, 1, 1); // Enhanced
      }
      else {
        aiPlanAddUnitType(dockPlan, gEconUnit, 1, 1, 1);
      }
   
      aiPlanSetNumberVariableValues(dockPlan, cBuildPlanDockPlacementPoint, 2, true); // Changed 2 to 7 in order to increase number of docks...
   ...
   //       gMonitorMaintain = createSimpleMaintainPlan(gMonitorUnit, 2, false, baseID, 1);
	// 	 gBattleshipMaintain = createSimpleMaintainPlan(gBattleShipUnit, 1, false, baseID, 1);
   //    }
   // }
       ...
       (age >= cAge3))
   {
	  ...
	  int battleshipValue = kbUnitCostPerResource(gBattleShipUnit, cResourceWood) +
                            kbUnitCostPerResource(gBattleShipUnit, cResourceGold);
      ...
      while (gNetNavyValue > 0.0)
      {
		 ...
		 if (numberBattleships < battleshipLimit)
         {
            numberBattleships++;
            gNetNavyValue -= battleshipValue;
            continue;
         }
         break;
      }
   ...
   {
      aiPlanSetVariableInt(gBattleshipMaintain, cTrainPlanNumberToMaintain, 0, numberBattleships);
   }
      
   /////////////////   
   // Repair part.
   // We re-purpose a combat defend plan to function as a repair plan by basically forcing a unit next to a Dock.
   ///////////////// 
...
void setUnitPickerDisabledUnits(int upID = -1)
{
   ...
   if (cMyCiv == cCivFrench)
   {
      ...
      kbUnitPickSetPreferenceFactor(upID, cUnitTypeNativeScout, 0.0);
   }
...
void setUnitPickerPreference(int upID = -1)
{
   ...
   { // We have an ally or trigger command, so bias everything for that one unit
      ...
      // Prevent the production of less-effective archaic units in later ages...
      if (kbGetAge() > cAge2) {
         kbUnitPickSetPreferenceFactor(upID, cUnitTypeAbstractArchaicInfantry, 0.0);
      }

      kbUnitPickSetPreferenceFactor(upID, cUnitTypeAbstractInfantry, 0.8); // Range 0.0 to 1.0.
      kbUnitPickSetPreferenceFactor(upID, cUnitTypeAbstractArtillery, 0.1);
      // Germans should make mercenaries...
      if (cMyCiv == cCivGermans) {
         kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeMercenary, 0.4);
      }
   ...
   if (cvPrimaryArmyUnit >= 0)
   {
      ...
      if (cvSecondaryArmyUnit < 0)
      {
         ...
         // Germans should make mercenaries...
         if (cMyCiv == cCivGermans) {
            kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeMercenary, 0.25);
         }
         kbUnitPickSetPreferenceFactor(upID, cUnitTypeAbstractCavalry, 0.6);
         if (cMyCiv == cCivXPAztec)
         {
   ...
   // Germans should make mercenaries...
   if (cMyCiv == cCivGermans) {
      kbUnitPickSetPreferenceFactor(gLandUnitPicker, cUnitTypeMercenary, 0.55 + (btBiasCav / 2.0));
   }
   ...
   // Favor proper units - Enhanced AI
   setUnitPickerRoyalGuardPreference(upID);
   setUnitPickerSpecialUnitPreference(upID);
   setUnitPickerArchaicUnitFilter(upID);
...
minInterval 30
{
   ...
   // French should avoid Native Scouts since they waste resources on them.
   if (cMyCiv == cCivFrench)
   {
      kbUnitPickSetPreferenceFactor(nativeUPID, cUnitTypeNativeScout, -1.0);
   }
   ...
   for (i = 0; < numberUnitTypes)
   {
      ...
      // French should not train Native Scouts.
      if ((cMyCiv == cCivFrench) && (trainUnitID == cUnitTypeNativeScout))
      {
         trainUnitID = -1;
      }
...
minInterval 45
{
   ...
   kbUnitPickSetPreferenceFactor(influenceUPID, cUnitTypedeSebastopolMortar, 0.5);
   ...
   kbUnitPickSetPreferenceFactor(influenceUPID, cUnitTypeMortar, 0.5); // Reduced since they make too many of these...
