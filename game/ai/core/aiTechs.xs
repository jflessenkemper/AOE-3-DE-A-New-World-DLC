...
int chooseEuropeanPolitician()
{
   ...
   switch (age)
   {
      case cAge1:
      {
         for (i = 0; i < numPoliticianChoices; i++)
         {
            ...
            // Include certain politicians. - Enhanced
         if ((politician == cTechDEPoliticianImam && cMyCiv == cCivOttomans) ||
            (politician == cTechDEPoliticianLogisticianPortuguese && cMyCiv == cCivPortuguese) ||
            (politician == cTechDEPoliticianLogisticianFrench && cMyCiv == cCivFrench) ||
            (politician == cTechDEPoliticianLogisticianSwedish && cMyCiv == cCivDESwedish) ||
            (politician == cTechDEPoliticianLogisticianColonialDutch && cMyCiv == cCivDutch) ||
                (politician == cTechPoliticianQuartermaster && (cMyCiv != cCivOttomans &&
                cMyCiv != cCivPortuguese && cMyCiv != cCivFrench && cMyCiv != cCivDESwedish &&
                cMyCiv != cCivDutch)) ||
                (politician == cTechPoliticianPhilosopherPrince && (cMyCiv != cCivOttomans &&
                cMyCiv != cCivPortuguese && cMyCiv != cCivFrench && cMyCiv != cCivDESwedish &&
                cMyCiv != cCivDutch)))
            {
               debugTechs("" + kbGetTechName(politician));
               xsArraySetInt(ageUpPoliticians, numValidPoliticians, politician);
               return (xsArrayGetInt(ageUpPoliticians, numValidPoliticians));
               numValidPoliticians++;
         }
      }
      ...
      case cAge2:
   {
         for (i = 0; i < numPoliticianChoices; i++)
      {
         ...
         {
            // Enhanced
            if ((politician == cTechDEPoliticianMercContractor && cMyCiv == cCivOttomans) || 
                //(politician == cTechDEPoliticianMercContractorFortressBritish && cMyCiv == cCivBritish) ||
                //politician == cTechDEPoliticianMercContractorFortressSwedish && cMyCiv == cCivDESwedish ||
                (politician == cTechPoliticianExiledPrince && cMyCiv != cCivOttomans) ||
                (politician == cTechPoliticianBishopFortress)) {
               debugTechs("" + kbGetTechName(politician));
               xsArraySetInt(ageUpPoliticians, numValidPoliticians, politician);
               numValidPoliticians++;
            } else if (cMyCiv != cCivBritish && cMyCiv != cCivFrench && cMyCiv != cCivDutch &&
                        cMyCiv != cCivGermans && cMyCiv != cCivPortuguese && cMyCiv != cCivRussians &&
                        cMyCiv != cCivDESwedish  && cMyCiv != cCivOttomans) {
               debugTechs("" + kbGetTechName(politician));
               xsArraySetInt(ageUpPoliticians, numValidPoliticians, politician);
               numValidPoliticians++;
            }
      ...
      case cAge4:
         ...
         if ((gANWAllowRevolt == true) &&
            ...
            // Give a higher chance to revolt as Barbary States for Ottomans on sea maps.
            if (cMyCiv == cCivOttomans && gStartOnDifferentIslands == true &&
            kbGetTechName(revolution) == "DERevolutionBarbaryStates" && 
            randomizer > 1) {
               return cTechDERevolutionBarbaryStates;
            }
         }
...
maxInterval 15
{
   ...
   // (age >= cAge3) && (cvOkToTrainArmy == true) && (militaryPopLimit / 6 > currentMilitaryPop) ||
   // We must plan to age up if we have not done so two minutes after the first person has aged up.
   if ((gExcessResources == false && age > cAge2) && (gAgeUpTime + 3 * 60 * 1000 > time)) // age >= cAge2
   {
      bool higherAgePlayerExists = false;

      for (int i = 0; i < cNumberPlayers; i++)
      {
         if (i == cMyID) continue;

         int playerAge = kbGetAgeForPlayer(i);

         if (playerAge > age)
         {
            higherAgePlayerExists = true;
            break;
         }
      }
      if (higherAgePlayerExists == false) {
   ...
   if ((firstAgeUpTime + 1 * 60 * 1000 < time) ||
       ((firstAgeUpTime + 1 * 60 * 1000 < time) &&
        (((gDefenseReflexBaseID != mainBaseID) && (militaryPopLimit / 2 < currentMilitaryPop
         && (btRushBoom > 0.0 && btOffenseDefense < 0.5))) ||
   ...
   if (age <= cAge4)
   {
      if (btRushBoom <= 0.0 && btOffenseDefense < 0.5)
      {
         ageUpPriority = ageUpPriority + 5; // 16
      }
      else if (btRushBoom <= 0.0)
      {
         ageUpPriority = ageUpPriority + 10; // 22
      }
   }

   // We don't want to stay behind in age-ups, nor do we want to take too long. For Hard mode only.
   if ((((firstAgeUpTime < time) || ((gAgeUpTime + 4 * 60 * 1000) < time)) && 
      age == cAge2) || (((firstAgeUpTime < time) || ((gAgeUpTime + 5 * 60 * 1000) < time)) && 
      age == cAge3) || ((firstAgeUpTime < time) && age == cAge4)) {
      ageUpPriority = 95;
   }

   // if ((firstAgeUpTime + 2 * 60 * 1000 < time)) {
   //    ageUpPriority = 95;
   // }

   // float startingHandicap = 1.0;
   // if (age < cAge4 && (cDifficultyCurrent > cDifficultyHard)) {
   //    // Hardest...
   //    if ((cDifficultyCurrent == cDifficultyExpert)) {
   //       kbSetPlayerHandicap(cMyID, startingHandicap * 1.50);
   //    } else if ((cDifficultyCurrent == cDifficultyExpert) && age == cAge3) {
   //       kbSetPlayerHandicap(cMyID, startingHandicap * 1.40);
   //    } else if ((cDifficultyCurrent == cDifficultyExpert) && age >= cAge4) {
   //       kbSetPlayerHandicap(cMyID, startingHandicap * 1.20);
   //    }
   //    // Extreme...
   //    if ((cDifficultyCurrent == cDifficultyExtreme) && age < cAge3) {
   //       kbSetPlayerHandicap(cMyID, startingHandicap * 1.8);
   //    } else if ((cDifficultyCurrent == cDifficultyExtreme) && age == cAge3) {
   //       kbSetPlayerHandicap(cMyID, startingHandicap * 1.95);
   //    } else if ((cDifficultyCurrent == cDifficultyExtreme) && age >= cAge4) {
   //       kbSetPlayerHandicap(cMyID, startingHandicap * 1.45);
   //    }
   // }
