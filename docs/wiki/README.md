...
- [Unit index](data-layer/unit-index.md) — every createable unit (stats + which nations can build it + icon), auto-generated by `tools/validation/build_unit_index.py`
...
- [UI surface map](validation/ui-surfaces.md) — verified 1920×1080 click coords for every HUD surface, ESC menu, diplomacy panel, postgame tabs, minimap; known-broken F-keys; canonical capture surface list
- [Nations and buildings map](validation/nations-and-buildings.md) — all 44 ANW civ tokens → culture; 12 culture building-roster templates; per-civ overrides; screenshot coverage status (RioGrande + BajaCalifornians have zero captures)
